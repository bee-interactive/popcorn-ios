...
class PHPSchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    private func extractRequestData(from request: URLRequest,
        ...
        if let url = request.url {
            ...
            getParameters = urlComponents?.query
        }
        ...
        // Extract POST data if method is POST
        var postParameters: String?
        if method.uppercased() == "POST", let httpBody = request.httpBody {
        ...
        let requestData = RequestData(getParameters: getParameters ?? "",
